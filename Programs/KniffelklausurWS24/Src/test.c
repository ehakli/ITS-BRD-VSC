...
void testIO(void){
   ...
   delay(DELAY);
	 ...
	 // angeschaltet, die dem gr�ssten Taster entspricht.
	 while(1){
	    int max = 0;
...
void testDisplay(void){
   ...
   // Das Display ist blau
	 ...
	 // Alle W�rfel sind magenta. Von rechts nach links haben sie die Werte 1 2 3 4 5
	 for (int die = 0; die < NO_DICE; die++){
	    displayDie(die, die+1, DIE_COLOR_FIXED);
	    delay(DELAY);
	 }
	 
	 // Die beiden linken W�rfel sind gelb und haben 6 Augen
	 displayDie(4, 6, DIE_COLOR_WIN);
	 delay(DELAY);
	 displayDie(3, 6, DIE_COLOR_WIN);
	 delay(DELAY);
	 
	 // Der rechte W�rfel ist gr�n und hat 3 Augen
...
static uint8_t testInfoDice[NO_DICE];   // Hilfsspeicher, der die aktuellen W�rfelwerte speichert
...
static void testIsStreet2(void){
	 ...
	 // fixiere die W�rfel von startVal an, bis eine Strasse vorliegt
